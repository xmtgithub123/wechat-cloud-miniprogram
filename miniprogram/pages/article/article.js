...
Page({
  data: {
    article: "",
    reArticleList: [], // 推荐阅读文章列表
    isShowPage: false,
    showFloatShareAfterHeight: -1,// 显示悬浮按钮的滚动高度
    isShowFloatShare: false, // 是否显示悬浮分享按钮
    isLike: false, // 是否点赞
  ...
  onLoad: function (options) {
    ...
    let id = options.scene ? options.scene : options.id ? options.id  : 'W_-mpnahafTeBPdK'
    id = id.replace(/\s/g, '')
    ...
    cloudApi.getArticleById(id).then(res => {
      ...
      article.createtime = moment(article.createtime).fromNow()
      WxParse.wxParse('content', 'html', content, this, 20, () => {
        ...
        let st = setTimeout(() => {
          ...
          query.select('.article-view-info').boundingClientRect().exec(rect => {
            let top = rect[0].top + rect[0].height
            this.setData({
              showFloatShareAfterHeight: top
            })
          })
        })
      })
      // 判断是否已经点赞
      this.judgmentIsLikeThisArticle(article._id)
      // 显示推荐阅读
      this.getRecommendationArticleList(article._id)
      this.setData({
        article
      }, () => {
        // 添加文章阅读数
        this.addArticleViewCount()

        this.downloadArticleHeaderImg()
        this.downloadArticleQRCode()
      })
      let articleWebViewURL = 'article.webViewURL'
      this.setData({
        [articleWebViewURL]: `/pages/h5/h5?url=${article.webViewimUrl}`
      })
    })
  },
  ...
  downloadArticleHeaderImg() {
    ...
    wx.cloud.downloadFile({
      ...
      success(res) {
        console.log('headerImg:' + res.tempFilePath)
        self.setData({
  ...
  downloadArticleQRCode() {
    let article = this.data.article
    const self = this
    wechatApi.getShareQRCode('pages/article/article', article._id).then(res => {
      if (res.code === 0) {
  ...
  drawShareImg(userInfo, article, shareInfo) {
    ...
    this.getDrawImgSize(headerImgSize, shareInfo.headerImg).then(res => {
      ...
      ctx.fillText('文章', tagPos.dx, tagPos.dy)
      // 画标题
      ctx.fillStyle = 'black'
      ctx.font = '48px Arial'
  ...
  tapImgAutoSave(e){
    ...
    wx.cloud.downloadFile({
      ...
      success(res) {
        console.log(res)
        originImgUrl = res.tempFilePath
        console.log('originImgUrl:' + originImgUrl)
        wx.saveImageToPhotosAlbum({
          filePath: originImgUrl,
          success(res) {
            wx.showToast({
              title: '图片已保存',
              duration: 2000
  ...
  onPageScroll(e) {
    ...
    let afterHeight = this.data.showFloatShareAfterHeight
    if (afterHeight < 0) return
    let isShowFloatShare = this.data.isShowFloatShare
    if (scrollTop > afterHeight && !isShowFloatShare) {
      this.setData({
        isShowFloatShare: true
      })
    } else if (scrollTop <= afterHeight && isShowFloatShare) {
      this.setData({
        isShowFloatShare: false
      })
    }
  },
  // 返回
  ...
  onShareAppMessage(object) {
    ...
    if (!submiting && !shareInfo.includes(article._id)) {
      submiting = true
      cloudApi.updateArticleShareNum(article._id).then(res => {
        this.setData({
          'article.shareNum': article.shareNum ? article.shareNum + 1 : 1
        }, () => {
          submiting = false
          // 将此文章id存入用户分享信息中
          shareInfo.push(article._id)
          wx.setStorageSync(localStorageKey.SHARE_INFO, shareInfo)
        })
      })
    }
    // https://7978-yx-8d220c-1258124532.tcb.qcloud.la 此链接为云控制台中图片的https下载地址的前缀
    // 由于分享的图片链接不支持云id，因此使用此地址
    return {
      title: article.title,
      path: `/pages/article/article?id=${article._id}&from=share`,
      imageUrl: 'https://7978-yx-8d220c-1258124532.tcb.qcloud.la' + article.cover.slice(article.cover.lastIndexOf('/')) 
    }
  },
  // 切换是否喜爱文章
  switchLikeThisArticle() {
    let article = this.data.article
    ...
    let isLike = false
    if (!likeInfo.includes(article._id)) {
      isLike = true
    }
    cloudApi.updateArticleLikeNum(article._id, isLike).then(res => {
      if (res.result.stats.updated) {
        ...
        this.setData({
          isLike,
          'article.likeNum': (article.likeNum || article.likeNum === 0) ? article.likeNum + num : 1
        }, () => {
          // 将此文章id存入用户点赞信息中
          if (isLike) {
            likeInfo.push(article._id)
          } else {
            likeInfo.splice(likeInfo.indexOf(article._id), 1)
          }
  ...
  // 获取推荐文章
  getRecommendationArticleList(id) {
    id = id.replace(/\s/g, '')
    return cloudApi.getRecommendationArticleList(id).then(res => {
      if (res.result) {
        let data = res.result.map((val,index) => {
          val.cover = `/resource/imgs/${index+1}.jpg`
          return val
        })
        this.setData({
          reArticleList: data
  ...
  addArticleViewCount() {
    ...
    let index = localReadInfo.findIndex(val => val._id == article._id)
    if (index < 0) {
      cloudApi.updateArticleViewCount(article._id).then(res => {
        if (res.result.stats.updated) {
          this.setData({
            'article.viewCount': article.viewCount ? article.viewCount + 1 : 1
          }, () => {
            // 存储用户阅读信息
            localReadInfo.push({
              _id: article._id,
